module.exports = {
  validateRegistration: (req, res, next) => {
    ...
    if (gender !== 'male' && gender !== 'female' && gender !== 'other') return res.send({ error: true, message: 'please select your gender', data: null })
    if (age < 18) return res.send({ error: true, message: 'users age must be 18 or above', data: null })
    if (city.length < 1) return res.send({ error: true, message: 'invalid city name', data: null })
    next()
  }
}
